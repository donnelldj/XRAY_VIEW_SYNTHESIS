...
# ------------------------- CT loading (match prepare_dataset_resize.py) -------------------------
...
def read_ct_preprocessed_zyx(
    ...
    img = reorient_to_canonical(img, orientation=canonical_orient)
...
class NpzAPLatCTDataset(Dataset):
    ...
    def __init__(
        ...
        self.canonical_orient = canonical_orient
    ...
    def __getitem__(self, idx: int) -> Sample:
        ...
        if self.load_ct:
            ...
            ct = read_ct_preprocessed_zyx(
                ...
                canonical_orient=self.canonical_orient,
            )  # (Z,Y,X) in 0..1
...
def forward_project_ct_to_lat(
    ...
    *,
    ...
    mx = lat.amax(dim=(-2, -1), keepdim=True)
...
def main():
    ...
    print(f"[info] target_zyx={target_zyx}  hu_clip={hu_clip}  canonical_orient={args.canonical_orient}")
    print(f"[info] lat_transform: invert={args.lat_invert} rot_k={args.lat_rot_k} flip_ud={args.lat_flip_ud} flip_lr={args.lat_flip_lr}")
    ...
    ds_train = NpzAPLatCTDataset(
        ...
        hu_clip=hu_clip,
        canonical_orient=args.canonical_orient,
        load_ct=True,
    )
